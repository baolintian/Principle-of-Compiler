...
extern struct _fieldList drop_FList[][50];
extern int drop_pFList[];
extern char* drop_table_name[];
extern int drop_table_number;
//新增加的临时变量，目的是为了增加多表的查询功能。
extern struct ast_wordList* tempp;
extern struct _expr *drop_where_tree[];
...
%union {
	...
	struct ast_Updatecol *updatecol;
...
%type <updatecol>  update_asgn_list
...
delete_stmt: DELETE FROM table_references opt_where{
        drop_table_name[drop_table_number] = $3->name;
        drop_pFList[drop_table_number] = pFList;
        drop_where_tree[drop_table_number] = $4;
        for(int i=0; i<pFList; i++){
            //printf("%s %d\n", FList[i].name, FList[i].intval);
            drop_FList[drop_table_number][i] = FList[i];
        }
        drop_table_number++;
        pFList = 0;
    }
...
update_stmt: UPDATE table_references 
		     ...
		     opt_where{
                  ...
                  //printf("hah test---%s\n", $4->name);
                   // traverse($4);
                   process_update($2, $4, $5);
                   pFList = 0;
		     }
	;

update_asgn_list: NAME CMP expr{
    $$ = (struct ast_UpdateCol*)malloc(sizeof(struct ast_UpdateCol));
    $$ = create_update_col($1, $3);


}
 /*	| NAME '.' NAME CMP expr */

	| update_asgn_list ',' update_asgn_list{
	    $$  = (struct ast_UpdateCol*)malloc(sizeof(struct ast_UpdateCol));
        $$ = merge_update_col($1, $3);
        //derefering problem;
        //printf("ttt---%s\n", $1->name);
	}
